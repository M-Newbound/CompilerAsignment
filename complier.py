...
class Scanner:
    ...
    def skip_white_space(self):
        ...
        '''
        isInBounds = lambda : self.current_char_index < len(self.input_string)
        isWhiteSpace = lambda : self.input_string[self.current_char_index].isspace()

        while isInBounds() and isWhiteSpace():
    ...
    def get_token(self):
        ...
        if self.current_char_index >= len(self.input_string) : return None
        ...
        if token is None : return self.no_token()
    ...
    def lookahead(self):
        ...
        '''
        return self.current_token[0] if self.current_token is not None else None
    ...
    def consume(self, *expected_tokens):
        ...
        if token not in expected_tokens : self.unexpected_token(token, expected_tokens)
...
class If_Else_AST:
    ...
    def __repr__(self):
        return 'if ' + repr(self.condition) + ' then ' + \
                repr(self.then) + ' end' + \
                'else then ' + repr(self.elseThen) + ' end'
...
class Boolean_Term_AST():
    ...
    def false_code(self, false_label):
        result = ''
        for i in range(len(self.factors)):
            factor = self.factors[i]

            result += factor.false_code(false_label)
...
class Boolean_Expression_AST():
    ...
    def false_code(self, false_label):
        ...
        true = label_generator.next()
        for i in range(len(self.terms)):
            term = self.terms[i]

            if i == len(self.terms) - 1:
                result += term.false_code(false_label)
                result += "goto" + " " + true + '\n'
                
            else:
                false = label_generator.next()
                result += term.false_code(false)
                result += "goto" + " " + true + '\n'
                result += false + ":" + '\n'
...
class Boolean_Factor_AST():
    ...
    def true_code(self, label):
        if self.t is True:
            return self.comp.true_code(label)
        return self.comp.false_code(label)
...
def boolean_expression():
    ...
    while scanner.lookahead() == Token.OR:
        ...
        trm = boolean_term()
        result.append(trm)
    return Boolean_Expression_AST(result)
...
def boolean_factor():
    if scanner.lookahead() == Token.NOT:
        ...
        return Boolean_Factor_AST(comparison())
